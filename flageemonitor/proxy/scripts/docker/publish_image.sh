...
IMAGE_NAME="${IMAGE_NAME:-ghcr.io/flagee-cloud/flageemonitor-client}"
TAG="${TAG:-$(git -C "$ROOT_DIR" rev-parse --short HEAD)}"
GHCR_USER="${GHCR_USER:-}"
GHCR_TOKEN="${GHCR_TOKEN:-}"
RUNTIME_DIR="$ROOT_DIR/proxy/runtime"

DOCKERFILE="$ROOT_DIR/flageemonitor/proxy/docker/Dockerfile"

if [[ ! -f "$DOCKERFILE" ]]; then
  echo "Dockerfile nao encontrado em $DOCKERFILE"
  exit 1
fi
